        ...
        <div className="animate-enter" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div className="stat-card" style={{ borderBottomColor: '#00ccff' }}>
                <h2 className="section-title" style={{ borderColor: '#00ccff', marginTop: 0 }}>
                    <Cloud size={24} style={{ marginRight: '0.5rem', color: '#00ccff' }} />
                    アカウント・クラウド同期
                </h2>
                <div style={{ padding: '0.5rem 0', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {auth ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.8rem', flexWrap: 'wrap', backgroundColor: '#111', padding: '1rem', border: '1px solid #333', borderRadius: '8px' }}>
                                <div>
                                ...
                                <button onClick={() => { if(window.confirm('ログアウトしますか？')) logout(); }} className="btn-smash" style={{ background: '#444', padding: '0.6rem 1rem', width: 'auto', flexShrink: 0 }}>
                            ...
                            <div style={{ backgroundColor: 'rgba(0, 204, 255, 0.1)', padding: '1.5rem', border: '1px solid #00ccff', borderRadius: '8px', marginTop: '1rem' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                    <input 
                                        ...
                                        style={{ flex: '1 1 200px', minWidth: 0, padding: '0.8rem', background: '#000', color: showObsUrl ? '#00ccff' : 'var(--text-muted)', border: '1px solid #00ccff', fontFamily: 'monospace' }}
                                    />
                                    <button onClick={() => setShowObsUrl(!showObsUrl)} className="btn-smash" style={{ background: '#333', padding: '0.6rem 0.9rem', width: 'auto', flex: '0 0 auto' }}>
                                        <div style={{ transform: 'skewX(20deg)', display: 'flex', alignItems: 'center' }}>
                                            {showObsUrl ? <EyeOff size={20} /> : <Eye size={20} />}
                                        </div>
                                    </button>
                                    <button onClick={copyObsUrl} className="btn-smash" style={{ background: '#00ccff', padding: '0.6rem 1rem', width: 'auto', flex: '1 1 auto' }}>
                                        <div style={{ transform: 'skewX(20deg)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                            <Copy size={20} /> コピー
                                        </div>
                                    </button>
            ...
            <div className="stat-card" style={{ borderBottomColor: 'var(--smash-yellow)' }}>
                <h2 className="section-title" style={{ borderColor: 'var(--smash-yellow)', marginTop: 0 }}>
                    <Image size={24} style={{ marginRight: '0.5rem', color: 'var(--smash-yellow)' }} />
                    背景画像の変更
                </h2>
                <div style={{ padding: '0.5rem 0', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.8rem' }}>
                        {backgrounds.map(bg => (
                            <div
                                ...
                                style={{
                                    border: backgroundImage === bg.url ? '3px solid var(--smash-yellow)' : '3px solid transparent',
                                    borderRadius: '8px',
                                    overflow: 'hidden',
                                    cursor: 'pointer',
                                    transition: 'all 0.2s',
                                    transform: backgroundImage === bg.url ? 'scale(1.03)' : 'scale(1)',
                                    boxShadow: backgroundImage === bg.url ? '0 0 15px rgba(255,204,0,0.5)' : 'none'
                                }}
                            >
                                <img src={bg.url} alt={bg.name} loading="lazy" style={{ width: '100%', height: '80px', objectFit: 'cover', display: 'block' }} />
                                <div style={{ padding: '0.4rem', backgroundColor: '#111', color: 'white', fontSize: '0.8rem', textAlign: 'center', fontWeight: 'bold', lineHeight: 1.3 }}>
                                    {bg.name}
                                </div>
                            </div>
                        ))}
                    ...
                    <div style={{ marginTop: '1rem', borderTop: '2px solid #333', paddingTop: '1.5rem' }}>
                        ...
                        <button className="btn-smash" onClick={handleBgUploadClick} style={{ background: '#444', clipPath: 'none', padding: '0.8rem 1.5rem' }}>
                            <div style={{ transform: 'skewX(20deg)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div className="stat-card" style={{ borderBottomColor: 'var(--win-color)' }}>
                <h2 className="section-title" style={{ borderColor: 'var(--win-color)', marginTop: 0 }}>
                    <Database size={24} style={{ marginRight: '0.5rem', color: 'var(--win-color)' }} />
                    ローカル バックアップ
                </h2>
                <div style={{ padding: '0.5rem 0', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    <p style={{ color: 'var(--text-muted)' }}>ローカルPCに現在のすべての対戦記録をJSONファイルとして保存、または復元します。</p>
                    <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                        <button onClick={handleExport} className="btn-smash" style={{ background: 'var(--win-color)', flex: '1 1 220px' }}>
                            <div style={{ transform: 'skewX(20deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                                <Download size={20} />PCへ保存 (エクスポート)
                            </div>
                        </button>
                        <input type="file" accept=".json" onChange={handleFileChange} ref={fileInputRef} style={{ display: 'none' }} />
                        <button onClick={handleImportClick} className="btn-smash" style={{ background: '#333', flex: '1 1 220px' }}>
                            <div style={{ transform: 'skewX(20deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
